...
class YandexCloudProvider {
    ...
    async updateFunction(request) {
        ...
        if (request.bucketName) {
            const objectNameArr = request.package.artifact.split('/')
            Object.assign(requestProps, {
                package: {
                    bucketName: request.bucketName,
                    objectName: objectNameArr[objectNameArr.length-1]
                }
            })
            this.serverless.cli.log(`Uploading source ${requestProps.package.objectName} to ${requestProps.package.bucketName}`)
            const file = await readFile(request.code)
            await this.uploadtoS3Bucket({
                ...requestProps.package,
                file
            })
            this.serverless.cli.log(`Upload completed`)
        } else {
            Object.assign(requestProps, {
        ...
        const operation = await this.functions.createVersion(requestProps);
        return operation.completion(this.session);
    }
    ...
    async uploadtoS3Bucket(request){
        await this.initConnectionsIfNeeded();
        await this.initAwsSdkIfNeeded();
        const params = {
